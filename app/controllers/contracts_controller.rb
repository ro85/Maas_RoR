class ContractsController < ApplicationController
      ...
      User.where(admin: false).each do |user|
        ...
        loop do                           
          ...
          when "Monday"
            ...
            loop_monitoring_shift(user, date, number_of_shift_per_day, @contract, start_hour)
          when "Tuesday"
  ...
  def loop_monitoring_shift(user, date, number_of_shift_per_day, contract, start_hour)               
    loop do               
      @monitoring_shift = MonitoringShift.new(user_id: user.id, date: date + @d, start_hour: start_hour + (@i * 3600), duration: 1, week_number: (date + @d).strftime('%W').to_i, contract_id: @contract.id)
